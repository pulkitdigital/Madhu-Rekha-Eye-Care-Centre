...
//       const response = await axios.get(
//         `${API_URL}/discharge-slips/discharge-ticket/${ticketId}`,
//       );
...
//       alert(
//         "Failed to fetch ticket details for editing: " +
//           (err.response?.data?.message || err.message),
//       );
//     }
//   };
...
const handleEdit = async (ticketId) => {
  ...
  const handleDelete = async (ticket) => {
    ...
    try {
      console.log("🗑️ Deleting ticket:", ticketId);
      const response = await axios.delete(
        `${API_URL}/discharge-slips/discharge-ticket/${encodeURIComponent(ticketId)}`,
      );
